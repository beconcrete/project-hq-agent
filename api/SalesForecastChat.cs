...
public class SalesForecastChat
{
    ...
    public SalesForecastChat(IHttpClientFactory httpFactory, IConfiguration config)
    {
        _httpFactory = httpFactory;
        _appId = config["APP_ID"] ?? "hqagents";
        _agentBaseUrl = config["CHAT_AGENT_BASE_URL"] ?? "";
        _agentKey = config["CHAT_AGENT_FUNCTION_KEY"] ?? config["CHAT_AGENT_KEY"] ?? "";
    }

    [Function("SalesForecastChat")]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "sales-forecast-chat")] HttpRequestData req,
        FunctionContext context)
    {
        ...
        using var agentReq = new HttpRequestMessage(
            HttpMethod.Post, $"{_agentBaseUrl}/api/sales-forecast-chat");
